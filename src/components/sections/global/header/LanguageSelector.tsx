  ...
  return (
    <div className={`relative ${!isMobile ? 'hidden sm:block' : 'w-full flex justify-center'}`}>
      ...
      <button
        ...
        className={`flex items-center justify-between gap-2 bg-[#333] hover:bg-[#444] border border-[#444] text-white transition-all cursor-pointer select-none ${isMobile
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.ul
            ...
            exit={{ opacity: 0, y: 10 }}
            className={`absolute z-50 bg-[#252525] border border-white/5 rounded-[12px] p-2 shadow-[0_10px_25px_rgba(0,0,0,0.5)] list-none flex flex-col ${isMobile
          ...
          >
            {languages.map((lang) => (
              <li
                ...
                className={`px-4 py-3 text-sm rounded-[8px] cursor-pointer transition-all duration-200 select-none ${language === lang.code
