...
export default function HeaderView({ lang, navLinks, partnerLabel }: HeaderProps) {
  ...
  return (
    <header className={`fixed top-0 w-full z-50 transition-all duration-500 ease-in-out backdrop-blur-md border-b border-white/5 ${isScrolled || isMobileMenuOpen ? "bg-black/90" : "bg-transparent"
      }`}>
      <div className="w-full max-w-[1920px] mx-auto px-24 py-4 flex items-center justify-between">
        ...
        <nav className="hidden md:flex gap-8 text-white text-sm font-medium">
          {navLinks.map((link, i) => (
            <Link key={i} href={link.href} className="hover:text-[#94CE00]">
              {link.label}
            </Link>
          ))}
        </nav>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden bg-black fixed inset-0 top-[65px] p-8 flex flex-col gap-8 text-white text-xl font-medium">
          {navLinks.map((link, i) => (
            <Link key={i} href={link.href} onClick={() => setIsMobileMenuOpen(false)}>
              {link.label}
            </Link>
          ))}
          <div className="flex gap-6 pt-4 border-t border-white/10 text-sm">
            <button onClick={() => handleLangChange("pt-br")} className={lang === "pt-br" ? "text-[#94CE00]" : ""}>PT</button>
            <button onClick={() => handleLangChange("es")} className={lang === "es" ? "text-[#94CE00]" : ""}>ES</button>
            <button onClick={() => handleLangChange("en")} className={lang === "en" ? "text-[#94CE00]" : ""}>EN</button>
          </div>
        </div>
      )}
    </header>
